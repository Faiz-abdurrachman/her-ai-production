...
function testSendCredentialEmail() {
  // NIK target test dibaca dari Script Properties (HERAI_QA_NIK) agar tidak
  // hardcode identitas akun di source. Fallback kosong: isi NIK manual saat run.
  var testNik = String(PropertiesService.getScriptProperties()
    .getProperty(QA_PARTICIPANT_PROPERTY_KEYS.nik) || '').replace(/\D/g, '');
  if (!testNik || testNik.length !== 16) {
    Logger.log('TEST NIK TIDAK VALID — set HERAI_QA_NIK di Script Properties');
    return;
  }
...
function fixCorruptedPassword() {
  var fixList = [
    // isi NIK peserta yang kolom generated_password-nya korup (#ERROR!)
    // contoh: '3275055902070004'
  ];
  // Password baru dibaca dari Script Properties agar tidak hardcode di source.
  // Set HERAI_RESET_PASSWORD=<password baru> di Script Properties sebelum run.
  var newPassword = PropertiesService.getScriptProperties()
    .getProperty('HERAI_RESET_PASSWORD') || '';
  if (String(newPassword).length < 8) {
    Logger.log('MINIMAL 8 KARAKTER — set HERAI_RESET_PASSWORD di Script Properties dulu');
    return;
  }
  var accounts = getRows(SHEETS.participantAccounts);
  var fixed = 0;
...
function broadcastRetryFailedEmails() {
  // Isi daftar email yang gagal terkirim di run sebelumnya (satu per baris).
  // Jangan commit email peserta asli ke source.
  var failedEmails = [
