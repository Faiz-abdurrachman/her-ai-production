    ...
    window.initAiEvaluationMateri = function () {
        ...
        if (list) {
            list.innerHTML = CHAPTERS.map(function (chapter, index) {
                ...
                return `<li data-chapter="${chapterNumber}"><span>${chapterNumber}</span><a href="javascript:void(0)" onclick="window.loadAiEvaluationChapter(${chapterNumber})">${escapeHtml(chapter.shortTitle)}</a><i class="far fa-circle"></i></li>`;
            }).join("");
        }
